...
public class GameTime : MonoBehaviour {
    ...
    public void Awake() {
		...
		if (setClockWhenStarting) { SetClock(startDay, startHour, startMinute, 0); }
	}
	...
	public void SetClock(int d, int h, int m, int s) {
		GameClockInSeconds = (d-FIRSTDAY) * (DAY * HOUR * MINUTE) + h * (HOUR * MINUTE) + m * MINUTE + s;
	}
	...
	public float DayLengthInSeconds {
		get
		{
		    if (_GameTimeScale > 0f) { return ((24f * 60f * 60f) / _GameTimeScale); }
		    return 0f;
		}
